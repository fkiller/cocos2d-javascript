...
(function () /** @lends cocos.commands.make.Compiler# */{
	...
	this.make = function () {
        ...
        code += fs.readFileSync(module_js, 'UTF-8');
	...
	this.makeResource = function (filename) {
        ...
        if (isCode) {
            ...
            data = JSON.stringify(fs.readFileSync(filename, 'UTF-8'));
        } else if (isImage) {
...
exports.description = 'Compile the cocos2d project into a single javascript file';
exports.run = function (opts) {
	var compiler = new Compiler('make.json');
	var code = compiler.make();

    var output = compiler.output;
    if (output) {
        sys.puts("Wrigint output to: " + output);
